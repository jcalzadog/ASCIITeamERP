...
namespace ERP.Dominio.Gestores
{
    class GestorOrder
    {
        ...
        public bool restarStock(decimal idOrder)
        {
            ...
            conector.setData("UPDATE PRODUCTS P SET P.STOCK = ( SELECT(P.STOCK - O.AMOUNT) FROM ORDERSPRODUCTS O WHERE P.IDPRODUCT = O.REFPRODUCT AND O.REFORDER ='" + idOrder + "') WHERE P.IDPRODUCT IN (SELECT PR.IDPRODUCT FROM PRODUCTS PR INNER JOIN ORDERSPRODUCTS OS ON PR.IDPRODUCT = OS.REFPRODUCT WHERE OS.REFORDER =" + idOrder + ")");
